        ...
        if(occupation == null || occupation.length() == 0 || occupation.equals("policeman")) {
            if(age > 40) {
                if(heartDisease && diabetes) {
                    this.healthInsurancePrice = 1100;   // branch 1
                } else if(!heartDisease && !diabetes) {
                    this.healthInsurancePrice = 360;    // branch 2
                } else if(!diabetes) {
                    this.healthInsurancePrice = 500;    // branch 3
                } else {
                    this.healthInsurancePrice = 400;    // branch 4
                }
            } else {
                this.healthInsurancePrice = 200;        // branch 5
            }
        } else {
            this.healthInsurancePrice = 350;            // branch 6
        }
    }
}
